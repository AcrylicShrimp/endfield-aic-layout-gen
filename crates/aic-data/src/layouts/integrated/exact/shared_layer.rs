...
use material_junction::{
    ...
    post_material_junction_restriction,
};
pub(in crate::layouts::integrated) use material_junction::{
    MaterialJunctionBuildCertificate, MaterialJunctionRestriction,
};
...
enum EndpointEncoding {
    ...
    FactoredSparseSupportBoundaryKeyAudit {
        ...
        material_separator_restrictions: Vec<MaterialSeparatorRestriction>,
...
impl EndpointEncoding {
    ...
    fn material_separator_restrictions(&self) -> &[MaterialSeparatorRestriction] {
        match self {
            Self::FactoredSparseSupportBoundaryKeyAudit {
    ...
    fn material_junction_restriction(&self) -> Option<&MaterialJunctionRestriction> {
        match self {
            Self::FactoredSparseSupportBoundaryKeyAudit {
                material_junction_restriction,
                ..
            } => material_junction_restriction.as_ref(),
            _ => None,
        }
    }

    fn material_junction_certificate_collector(
        &self,
    ) -> Option<&MaterialJunctionBuildCertificateCollector> {
        match self {
            Self::FactoredSparseSupportBoundaryKeyAudit {
...
pub(in crate::layouts::integrated) fn solve_factored_endpoints_fixed_dimensions_feasibility_only_with_prior_and_local_continuation(
    input: ModelInput,
    logistics_components: &ValidatedLogisticsComponentCatalog,
    time_limit: Option<Duration>,
    fixed_dimensions: FixedUsedDimensions,
...
pub(in crate::layouts::integrated) fn solve_sparse_support_endpoints_fixed_dimensions_feasibility_only_with_local_continuation_guarded_intersection_propagation(
    input: ModelInput,
    logistics_components: &ValidatedLogisticsComponentCatalog,
    time_limit: Option<Duration>,
    fixed_dimensions: FixedUsedDimensions,
...
pub(in crate::layouts::integrated) fn solve_sparse_support_endpoints_fixed_dimensions_coordinate_ports_prior_port_subset_ablation(
    ...
    solve_endpoints_fixed_dimensions_coordinate_ports_prior_overlap_ablation(
        ...
        fixed_ports,
        prior_solution,
        ReferenceAblationFixation::PriorOverlapPlacementsAndFacilityPortSubset(facility_port_mask),
    )
}
...
pub(in crate::layouts::integrated) fn solve_sparse_support_endpoints_boundary_key_continuation_and_material_separator_fixed_dimensions_coordinate_ports_prior_overlap_ablation(
    ...
    mut allowed_keys: Vec<i32>,
    continuation: EndpointContinuationRestriction,
    separators: Vec<MaterialSeparatorRestriction>,
) -> (
    IntegratedLayoutReport,
    Vec<BoundaryKeyBuildCertificate>,
    Vec<EndpointContinuationBuildCertificate>,
    ...
    let report = solve_endpoints_fixed_dimensions_coordinate_ports_prior_overlap_ablation(
        ...
        EndpointEncoding::FactoredSparseSupportBoundaryKeyAudit {
            ...
            endpoint_continuation_restriction: Some(continuation),
            endpoint_continuation_certificates: Some(SyncArc::clone(&continuation_certificates)),
            material_separator_restrictions: separators,
...
#[allow(clippy::too_many_arguments)]
pub(in crate::layouts::integrated) fn solve_sparse_support_endpoints_boundary_key_continuation_material_separator_and_junction_fixed_dimensions_coordinate_ports_prior_overlap_ablation(
    input: ModelInput,
    logistics_components: &ValidatedLogisticsComponentCatalog,
    time_limit: Option<Duration>,
    fixed_dimensions: FixedUsedDimensions,
    ...
    continuation: EndpointContinuationRestriction,
    separators: Vec<MaterialSeparatorRestriction>,
    junction: MaterialJunctionRestriction,
) -> (
    IntegratedLayoutReport,
    Vec<BoundaryKeyBuildCertificate>,
    Vec<EndpointContinuationBuildCertificate>,
    Vec<MaterialSeparatorBuildCertificate>,
    Vec<MaterialJunctionBuildCertificate>,
) {
    allowed_keys.sort_unstable();
    allowed_keys.dedup();
    assert!(
    ...
    let continuation_certificates = SyncArc::new(Mutex::new(Vec::new()));
    let separator_certificates = SyncArc::new(Mutex::new(Vec::new()));
    let junction_certificates = SyncArc::new(Mutex::new(Vec::new()));
    let report = solve_endpoints_fixed_dimensions_coordinate_ports_prior_overlap_ablation(
        input,
        logistics_components,
        time_limit,
    ...
    let separator = separator_certificates
        ...
        .expect("material-separator certificate collector is not poisoned")
        .clone();
    let junction = junction_certificates
        .lock()
        .expect("material-junction certificate collector is not poisoned")
        .clone();
    (report, boundary, continuation, separator, junction)
}

#[allow(clippy::too_many_arguments)]
pub(in crate::layouts::integrated) fn solve_sparse_support_endpoints_boundary_key_continuation_material_separator_and_junction_fixed_dimensions_coordinate_ports_prior_overlap_root_snapshot(
    input: ModelInput,
    logistics_components: &ValidatedLogisticsComponentCatalog,
    time_limit: Option<Duration>,
    fixed_dimensions: FixedUsedDimensions,
...
fn solve_with_endpoint_encoding(
    ...
    let mut tracked_row_selectors = Vec::new();
    ...
    let mut material_separator_probes = Vec::new();
    for restriction in endpoint_encoding.material_separator_restrictions() {
        match post_material_separator_restriction(
            ...
            Ok(probe) => material_separator_probes.push(probe),
            Err(diagnostic) => return IntegratedLayoutReport::invalid(diagnostic),
        }
    }
    let material_junction_probe =
        if let Some(restriction) = endpoint_encoding.material_junction_restriction() {
            match post_material_junction_restriction(
                &mut solver,
                &input,
                &model_terminals,
                &layers,
                restriction,
                endpoint_encoding.material_junction_certificate_collector(),
                tag,
            ) {
                Ok(probe) => Some(probe),
                Err(diagnostic) => return IntegratedLayoutReport::invalid(diagnostic),
    ...
    let formulation = match (
        ...
        connectivity_mode,
            ...
            (
                Some(ReferenceAblationFixation::PriorOverlapPlacementsAndFacilityPortSubset(_)),
                None,
                ConnectivityMode::None,
            ...
            (None, None, ConnectivityMode::None) => match (
                ...
                (EndpointEncoding::FactoredTrackedPositiveTable, Some(_), None, None) => {
                    "joint-shared-boundary-terminals-tracked-positive-table-v1-fixed-dimensions"
                }
                (EndpointEncoding::FactoredTrackedPositiveTable, None, None, None) => {
                    "joint-shared-boundary-terminals-tracked-positive-table-v1"
                }
                (EndpointEncoding::FactoredTrackedPositiveTable, _, _, _) => {
                    unreachable!("tracked positive-table research does not fix coordinates or ports")
                }
                (EndpointEncoding::FactoredSparseSupport(_), Some(_), None, None) => {
                    "joint-shared-boundary-terminals-sparse-support-v1-fixed-dimensions"
                }
                (EndpointEncoding::FactoredSparseSupport(_), None, None, None) => {
                    "joint-shared-boundary-terminals-sparse-support-v1"
                }
    ...
    let formulation = match &endpoint_encoding {
        ...
        EndpointEncoding::FactoredTrackedPositiveTable => {
            "joint-shared-v4-tracked-positive-table-endpoints-watched-demand-local-continuation-guarded-intersection-propagation"
        }
        EndpointEncoding::FactoredSparseSupport(_) => {
        ...
        EndpointEncoding::FactoredSparseSupportBoundaryKeyAudit {
            sparse_legal_domain: true,
            material_separator_restrictions,
            material_junction_restriction: Some(_),
            ..
        } if material_separator_restrictions.len() == 2
            && material_separator_restrictions[1]
                .selected_case_index
                .is_none() =>
        {
            "joint-shared-v4-sparse-support-endpoints-legal-boundary-key-source-continuation-material-separator-junction-row5-separator-control-watched-demand-local-continuation-guarded-intersection-propagation"
        }
        EndpointEncoding::FactoredSparseSupportBoundaryKeyAudit {
            sparse_legal_domain: true,
            material_separator_restrictions,
            ...
            "joint-shared-v4-sparse-support-endpoints-legal-boundary-key-source-continuation-material-separator-junction-row5-separator-partition-watched-demand-local-continuation-guarded-intersection-propagation"
        }
        EndpointEncoding::FactoredSparseSupportBoundaryKeyAudit {
            sparse_legal_domain: true,
            material_junction_restriction: Some(restriction),
            ..
        } if restriction.selected_case_index.is_none() => {
            "joint-shared-v4-sparse-support-endpoints-legal-boundary-key-source-continuation-material-separator-material-junction-control-watched-demand-local-continuation-guarded-intersection-propagation"
        }
        EndpointEncoding::FactoredSparseSupportBoundaryKeyAudit {
            sparse_legal_domain: true,
            material_junction_restriction: Some(_),
            ...
            "joint-shared-v4-sparse-support-endpoints-legal-boundary-key-source-continuation-material-separator-material-junction-partition-watched-demand-local-continuation-guarded-intersection-propagation"
        }
        EndpointEncoding::FactoredSparseSupportBoundaryKeyAudit {
            sparse_legal_domain: true,
            material_separator_restrictions,
            ..
        } if material_separator_restrictions.len() == 1
            && material_separator_restrictions[0]
                .selected_case_index
                .is_none() =>
        {
    ...
    let selected_facilities = match fixation {
        ReferenceAblationFixation::PriorOverlapPlacementsAndFacilityPortSubset(mask) => {
            let facility_ids = reference
                ...
                .collect::<BTreeSet<_>>();
            if facility_ids.len() > 63 {
                return Err(reference_mismatch(
                    "at most 63 prior facilities for a port-subset mask",
                    &facility_ids.len().to_string(),
                ));
            }
            Some(
                facility_ids
...
fn build_factored_terminals(
    ...
    endpoint_encoding: &EndpointEncoding,
    tracked_row_selectors: &mut Vec<DomainId>,
    ...
    match endpoint_encoding {
        ...
        EndpointEncoding::FactoredTrackedPositiveTable => {
            let rows = facility_values_by_port
                ...
                .collect::<Vec<_>>();
            tracked_row_selectors.extend(solver.post_tracked_table(
                ConstraintFamily::EndpointLink,
                vec![placement.choice, port_choice, facility_key],
                rows,
                tag,
        ...
        | EndpointEncoding::FactoredSparseSupportBoundaryKeyAudit { counters, .. } => {
            ...
            solver.record_global_constraint(
                ConstraintFamily::EndpointLink,
                [placement.choice, port_choice, facility_key],
            );
            let _ = solver
                .solver_mut()
                .add_propagator(SparseEndpointSupportPropagatorArgs {
                    name: format!("edge-{edge_index}-{endpoint_kind}-sparse-endpoint-support"),
                    variables: [placement.choice, port_choice, facility_key],
                    domain_values: [
                        (0..instance.candidates.len())
                            .map(|value| i32::try_from(value).expect("placement index fits i32"))
                            .collect(),
                        (0..ports.len())
                            .map(|value| i32::try_from(value).expect("port index fits i32"))
                            .collect(),
                        (0..=key_upper).collect(),
                    ],
                    rows,
