...
pub(super) fn run(
    ...
    let time_limit = NonZeroU64::new(time_limit_ms)
        .context("research fixed-dimension time_limit_ms must be positive")?;
    ensure!(
        !network_indices.is_empty(),
        "research fixed-dimension diagnosis requires at least one --network-index"
    );
    ...
    let report = solve_first_integrated_layout_phase_fixed_dimensions(
        &wiring,
        &facilities,
        &items,
        &transports,
    ...
    .map_err(|report| anyhow::anyhow!("fixed-dimension model preparation failed: {report:?}"))?;
    ...
    let html =
        render_integrated_layout_html_with_localization(&report.layout, localization.as_ref())
            .map_err(|diagnostic| {
                anyhow::anyhow!(
                    "fixed-dimension visualization failed with {}: {}",
                    diagnostic.code,
                    diagnostic.message
                )
            })?;
    write_bytes(
        ...
        html.as_bytes(),
        "fixed-dimension visualization",
    )?;
    let encoded = serde_json::to_vec_pretty(&report)
        .context("failed to serialize fixed-dimension diagnosis report")?;
    std::io::Write::write_all(&mut std::io::stdout().lock(), &encoded)
        .context("failed to write fixed-dimension diagnosis report")?;
    println!();
    Ok(true)
}
