...
use aic_data::layouts::{
    FacilityPlacementRequest, decompose_first_integrated_layout_phase_factored_requirements,
...
pub(super) fn run(
    ...
    let case_time_limit = NonZeroU64::new(case_time_limit_ms)
        .context("research requirement-cliff case_time_limit_ms must be positive")?;
    ...
    let report = decompose_first_integrated_layout_phase_factored_requirements(
        &wiring,
        &facilities,
        &items,
        &transports,
    ...
    .map_err(|report| {
        anyhow::anyhow!("factored-requirement model preparation failed: {report:?}")
    })?;
    ...
    for case in &report.cases {
        ...
        let html =
            render_integrated_layout_html_with_localization(&case.layout, localization.as_ref())
                .map_err(|diagnostic| {
                    anyhow::anyhow!(
                        "factored-requirement visualization failed with {}: {}",
                        diagnostic.code,
                        diagnostic.message
                    )
                })?;
        write_bytes(
            ...
            html.as_bytes(),
            "factored-requirement visualization",
        )?;
    }
    write_json(&output_dir.join("summary.json"), &report)?;
    let encoded = serde_json::to_vec_pretty(&report)
        .context("failed to serialize factored-requirement experiment report")?;
    std::io::stdout()
        .lock()
        .write_all(&encoded)
        .context("failed to write factored-requirement experiment report")?;
    println!();
    Ok(true)
}
