...
fn render_summary(report: &PossibleGraphConnectivityDiagnosisReport) -> Result<String> {
    let rows = report
        ...
        .map(|case| {
            ...
            format!(
                "<tr><td><a href=\"{name}.html\">{:?}</a></td><td>{:?}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td><td>{}</td></tr>",
                case.kind,
                case.outcome,
